...
class BenchmarkTableScanTest : public BaseTest {
 protected:
  void SetUp() override {
    ...
    start_ms = std::chrono::duration_cast<nanoseconds>(system_clock::now().time_since_epoch());
  }

  void TearDown() override {
    // Gets the time when the test finishes
    nanoseconds end_ms = std::chrono::duration_cast<nanoseconds>(system_clock::now().time_since_epoch());
    std::cout << " Starttime: " << std::to_string(start_ms.count()) << " - "
              << " Endtime: " << std::to_string(end_ms.count()) << " - "
              << " Time consumed: " << std::to_string(end_ms.count() - start_ms.count()) << std::endl;
  }
