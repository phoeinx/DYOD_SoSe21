...
class ReferenceSegment : public BaseSegment {
 public:
  ...
  const std::shared_ptr<const PosList>& pos_list() const { return _position_list; }
  ...
  size_t estimate_memory_usage() const final { return (sizeof(RowID) * _position_list->size()); }
