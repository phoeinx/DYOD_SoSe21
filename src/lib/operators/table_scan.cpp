...
std::shared_ptr<const Table> TableScan::_on_execute() {
  ...
  // If the input table already holds ReferenceSegments we set it to the referenced table of the input table
  ...
  return _create_reference_output_table(input_table, std::move(position_list), input_table->target_chunk_size(),
                                        input_table->column_count());
}
...
ValueID TableScan::_get_compare_value_id(ScanType type, ValueID upper_bound, ValueID lower_bound) {
  switch (type) {
    case ScanType::OpEquals: {
      // If upper_bound == lower_bound, value is not in dictionary and therefore doesn't appear in segment
      // Select nothing
    ...
    case ScanType::OpNotEquals: {
      // If upper_bound == lower_bound, value is not in dictionary and therefore doesn't appear in segment
      // Select everything, since everything is not equal to search value
      if (upper_bound == lower_bound) {
    ...
    case ScanType::OpGreaterThan: {
      ...
      // when the search value is not in the dictionary,
      // comparisons against the next smaller value give us the correct results,
    ...
    case ScanType::OpLessThanEquals: {
      ...
      // when the search value is not in the dictionary,
      // comparisons against the next smaller value give us the correct results,
      //    since lower_bound gives us here the first value > the search value,
      //    but we also want to include the next smaller one
      // when the search value is in the dictionary, we return its index
      if (upper_bound == lower_bound && upper_bound == ValueID{0}) {
        return EMPTY_RESULT;
      } else if (upper_bound == lower_bound && upper_bound == INVALID_VALUE_ID) {
...
std::vector<RowID> TableScan::_create_position_list(const std::shared_ptr<const Table>& input_table) {
  /**
   * To create the position_list with the RowIDs of all values that evaluate to true with the scan, we
   * 1. Iterate over each Chunk / the relevant Segment per Chunk referenced by the given ColumnID
   * 2. Check for the type of the Segment
   * 3. Specific for each type compare each value in the segment against the compare value with the given comparator
   *    to create position_list
   */
  ...
  resolve_data_type(input_table->column_type(_column_id), [&](auto type) {
    ...
    // Iterate over each Chunk / the relevant Segment per Chunk referenced by the given ColumnID
    for (auto chunk_id = ChunkID{0}; chunk_id < num_chunks; ++chunk_id) {
      ...
      if (typed_value_segment != nullptr) {
        // For ValueSegments simply compare each value of the value segment
        // against the compare value with given comparator
      ...
      } else if (const auto typed_dictionary_segment = std::dynamic_pointer_cast<DictionarySegment<Type>>(segment);
                 typed_dictionary_segment != nullptr) {
        /**
         * For Dictionary Segments use index-based comparing with value ids.
         * This is possible because dictionary values are stored in sorted manner and the total order of dictionary value ids 
         * therefore is based on the total order of values in the attribute vector.
         * Specifically, value_id_a <= value_id_b iff. value_a <= value_b.
         * We compare by first retrieving the correct compare_value_id to compare against given the ScanType and search value, 
         * and then comparing the ValueIDs in the attribute vector against it.
         */
        ...
        const auto search_value_id =
            _get_compare_value_id(_scan_type, typed_dictionary_segment->upper_bound(typed_search_value),
                                  typed_dictionary_segment->lower_bound(typed_search_value));
        ...
        if (search_value_id == SELECT_EVERYTHING) {
          for (auto cell_id = 0ul, typed_segment_size = attribute_value_ids->size(); cell_id < typed_segment_size;
               ++cell_id) {
            position_list.push_back(RowID{static_cast<ChunkID>(chunk_id), static_cast<ChunkOffset>(cell_id)});
          }
          continue;
        ...
        // Compare each value_id in the attribute vector to the calculated compare_value_id with comparator
        for (auto cell_id = 0ul, typed_segment_size = attribute_value_ids->size(); cell_id < typed_segment_size;
             ++cell_id) {
      ...
      } else if (const auto typed_reference_segment = std::dynamic_pointer_cast<ReferenceSegment>(segment);
                 typed_reference_segment != nullptr) {
        // For ReferenceSegments we materialize each referenced value and compare it to the search value
        ...
        for (const auto& row_id : *input_position_list) {
          const auto dereferenced_value = type_cast<Type>(
              (*(input_reference_table->get_chunk(row_id.chunk_id).get_segment(_column_id)))[row_id.chunk_offset]);
          if (comparator(dereferenced_value, typed_search_value)) {
...
std::shared_ptr<const Table> TableScan::_create_reference_output_table(const std::shared_ptr<const Table>& input_table,
  ...
  // Each ReferenceSegments refers to all rows that were evaluated to true by the scan
